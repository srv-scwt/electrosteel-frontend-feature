...
export async function generateMetadata() {
  return buildMetadataForPathname("/investors/amalgamation");
}
const CATEGORY = "extra-amalgamation";
...
const page = async ({ searchParams }) => {
  ...
  const amalgamation = await getExtraInvestorResponse({
    category: CATEGORY,
    page: currentPage,
    limit,
  });

  if (!amalgamation || amalgamation?.error) {
    return <SomethingWentWrong />;
  }

  const { sections, pagination } = amalgamation.data ?? {};
