...
export async function generateMetadata() {
  return buildMetadataForPathname("/investors/shareholder-information");
}
const CATEGORY = "extra-shareholder-information";
...
const page = async ({ searchParams }) => {
  ...
  const shareholderInformation = await getExtraInvestorResponse({
    category: CATEGORY,
    page: currentPage,
    limit,
  });

  if (!shareholderInformation || shareholderInformation?.error) {
    return <SomethingWentWrong />;
  }

  const { sections, pagination } = shareholderInformation.data ?? {};
