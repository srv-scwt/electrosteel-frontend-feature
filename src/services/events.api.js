...
export async function getEventPage(sectionName) {
  try {
    const response = await ServerFetch(`/frontend/events?type=${sectionName}`, { mode: "SSR" });
...
export async function getEventsDetailsBySlug(slug) {
  try {
    const response = await ServerFetch(`/frontend/events/slug/${slug}`, { mode: "SSR"});
